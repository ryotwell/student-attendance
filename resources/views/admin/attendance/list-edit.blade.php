...
@section('content')
    ...
    <x-common.component-card title="{{ $class->name }} - {{ $schedule->subject->name }} - {{ $date->format('d/m/Y') }} ({{ \App\Helpers\MenuHelper::getDayName($schedule->day) }} {{ $schedule->start_time }} - {{ $schedule->end_time }})">
        <form action="{{ route('attendance.list.update', [$class, $schedule, $date->format('Y-m-d')]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-end pt-4 border-t border-gray-200 dark:border-gray-700">
                <a href="{{ route('attendance.list.show', $class) }}" class="mr-3 px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    Kembali
                </a>
                <button type="submit"
