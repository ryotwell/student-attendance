...
@section('content')
    ...
    <div class="mx-auto max-w-6xl">
        <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-xl font-bold text-gray-800 dark:text-white">Catatan Kasus Siswa</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Seluruh catatan dari tim BK</p>
            </div>
        ...
        <form method="GET" class="mb-6 flex flex-col gap-3 sm:flex-row">
            ...
            <select name="category"
                ...
                @foreach ($categoryOptions as $key => $option)
                    <option value="{{ $key }}" @selected(request('category') === $key)>{{ $option['label'] }}</option>
                @endforeach
            </select>
            ...
            <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Siswa</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Kategori</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Dicatat Oleh</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-900">
                    @forelse ($cases as $case)
                        <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-800/60">
                            <td class="px-5 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $case->date->format('d/m/Y') }}</td>
                            <td class="px-5 py-3">
                                <div class="text-sm font-medium text-gray-800 dark:text-white">{{ $case->student->name }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ $case->student->xclass->name }}</div>
                            </td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $case->user->name }}</td>
                            <td class="px-5 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('bk.cases.by-student', $case->student) }}"
                                        class="rounded-lg px-3 py-1.5 text-xs font-semibold text-brand-600 hover:bg-brand-50 dark:text-brand-400 dark:hover:bg-brand-900/30">
                                        Riwayat
                                    </a>
                                    <a href="{{ route('bk.cases.edit', $case) }}"
                                        class="rounded-lg px-3 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800">
                                        Edit
                                    </a>
                                    <form action="{{ route('bk.cases.destroy', $case) }}" method="POST"
                                        onsubmit="return confirm('Hapus catatan kasus ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="rounded-lg px-3 py-1.5 text-xs font-semibold text-error-600 hover:bg-error-50 dark:text-error-400 dark:hover:bg-error-900/30">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
